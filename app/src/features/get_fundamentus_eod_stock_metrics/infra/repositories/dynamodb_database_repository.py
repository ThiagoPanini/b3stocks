...
class DynamoDBDatabaseRepository(IDatabaseRepository):
    ...
    def batch_save_stock_metrics(
        ...
        Args:
            stock_metrics_list (list[FundamentusStockMetrics]):
                List of stock metrics to save.
        """
        try:
            with FundamentusStockMetricsModel.batch_write() as batch:
                for _, item in enumerate(stock_metrics_list):
                    model = FundamentusStockMetricsModel(
                        **SerializationUtils.json_serialize(item)
                    )
                    batch.save(model)

            self.logger.info(
                f"Successfully batch saved {len(stock_metrics_list)} "
                f"stock metrics "
